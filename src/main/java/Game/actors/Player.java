 ...
 */
public abstract class Player extends Actor implements Comparator<Player>{
    ...
    public static Direction getMoveDirection(Vector playerPosition, Vector flagPosition) {
        ...
        int checkPositionX = playerPosition.getX() - flagPosition.getX();
        int checkPositionY = playerPosition.getY() - flagPosition.getY();


        if (checkPositionY == 0 && checkPositionX > 0) {
            return Direction.UP;
        } else if (checkPositionY == 0 && checkPositionX < 0) {
            return Direction.RIGHT;
        } else if (checkPositionY < 0) {
    ...
    public int getCurrentScore() {
        return capturedFlags + killedPlayers;
    }
    ...
    public int compare(Player first, Player second){
        return Integer.compare(first.getCurrentScore(), second.getCurrentScore());
    }
}
